...
# Import agent functions (your existing logic)
...
def run_agent_jobs(cfg, progress_queue: queue.Queue):
    """Run the selected scans according to cfg and emit progress messages to the queue."""
    try:
        ...
        if cfg.get("do_task_manager", False):
            ...
            progress_queue.put(("log", f"Applications: {len(task_data.get('applications', []))}"))
            send_scan_results(task_data, endpoint_path="tasks")
        ...
        # Optional: future network scan
        # if cfg.get("do_network_scan", False):
        #     progress_queue.put(("status", "Running network scan..."))
        #     network_results = ...  # call your network scan
        #     send_scan_results(network_results, endpoint_path="network-scan")
        #     progress_queue.put(("status", "Network scan sent."))

        progress_queue.put(("done", "All tasks completed successfully."))
...
class SummaryPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # custom event trigger when frame raises
        self._last_cfg = None
